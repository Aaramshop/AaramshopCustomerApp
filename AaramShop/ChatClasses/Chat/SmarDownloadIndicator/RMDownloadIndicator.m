...
- (void)updateWithTotalBytes:(CGFloat)bytes downloadedBytes:(CGFloat)downloadedBytes
{
    ...
    if (self.delegate && [self.delegate conformsToProtocol: @protocol(RMProgressDelegate)])
    {
        [self.delegate updateWithTotalBytes: bytes downloadedBytes: downloadedBytes];
    }
