...
-(void)createThumbURL:(NSString*)urlString
    ...
    if (image) {
        ...
        dispatch_async(dispatch_get_global_queue( DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^(void){
            ...
            //                    UIImage *image = [UIImage imageWithCGImage:im scale:1.0 orientation:UIImageOrientationRight];
            //
            if (image != nil) {
...
//        succeedBlock([NSURL fileURLWithPath:URLString],nil); // to show video from local
                                   ...
                                   if (!connectionError) {
                                       ...
                                       dispatch_async(dispatch_get_main_queue(), ^(void){
                                           if (jsonData.count) {
                                               ...
                                               [[SDWebImageManager sharedManager] downloadWithURL:[NSURL URLWithString:thumbURL] options:SDWebImageContinueInBackground progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
                                       ...
                                       dispatch_async(dispatch_get_main_queue(), ^(void){
                                           if (jsonData.count) {
                                               ...
                                               if ([jsonData firstObject][quality]) {
                                                   ...
                                                   [[SDWebImageManager sharedManager] downloadWithURL:[NSURL URLWithString:jsonData[0][quality]] options:SDWebImageContinueInBackground progress:nil completed:^(UIImage *image, NSError *error, SDImageCacheType cacheType, BOOL finished) {
