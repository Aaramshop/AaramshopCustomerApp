...
- (id)initWithFrame:(CGRect)frame{
    ...
    if (self) {
        ...
        _viewBackground = [[UIView alloc]initWithFrame:self.bounds];
        self.viewBackground.autoresizingMask = UIViewAutoresizingFlexibleHeight|UIViewAutoresizingFlexibleWidth;
        self.viewBackground.clipsToBounds = YES;
        self.viewBackground.layer.shadowOffset = CGSizeMake(0, 0);
        self.viewBackground.layer.shadowRadius = 1.0;
        self.viewBackground.layer.shadowColor = [UIColor grayColor].CGColor;
        self.viewBackground.layer.shadowOpacity = 0.5;
        self.viewBackground.layer.shadowPath = [UIBezierPath bezierPathWithRect:self.viewBackground.bounds].CGPath;
        self.viewBackground.layer.cornerRadius = 2.0;
        [self.contentView addSubview:self.viewBackground];
        
        _thumbnail = [UIImageView.alloc initWithFrame:CGRectMake(rect.origin.x+2, rect.origin.y+2, rect.size.width-4, rect.size.height-4)];
...
- (void)setImageOffset:(CGPoint)imageOffset
{
    ...
    // Grow image view
    CGRect frame = self.thumbnail.bounds;
    CGRect offsetFrame = CGRectOffset(frame, _imageOffset.x, _imageOffset.y);
    self.thumbnail.frame = offsetFrame;
}
@end
