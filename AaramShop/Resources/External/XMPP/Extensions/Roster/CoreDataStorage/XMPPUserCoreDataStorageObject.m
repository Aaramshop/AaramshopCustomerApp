...
@property(nonatomic,strong) NSDate *primitiveLastSeen;
...
@dynamic resources;
//nehaa 06-06-14
@dynamic lastSeen,primitiveLastSeen;
//end
...
- (void)recalculatePrimaryResourceDate:(XMPPPresence *)presence
{
	...
	if ([sortedResources count] > 0)
	{
		...
		if ([resource priority] >= 0)
		{
            ...
            {
				self.section = 1;
                double disPatchTimeStamp =  [[presence attributeStringValueForName:@"date"] doubleValue];
                NSDate *disPatchTimestamp = [[NSDate  alloc] initWithTimeIntervalSince1970: disPatchTimeStamp];
                [self setPrimitiveLastSeen:disPatchTimestamp];
            }
		}
	}
	...
	if (self.primaryResource == nil)
	{
        ...
        {
            NSDate *disPatchTimestamp = [[NSDate  alloc] initWithTimeIntervalSince1970: disPatchTimeStamp];
            [self setLastSeen:disPatchTimestamp];
        }
	}
...
- (void)updateWithPresence:(XMPPPresence *)presence streamBareJidStr:(NSString *)streamBareJidStr
{
    ...
    [self recalculatePrimaryResourceDate:presence];
}
...
- (id <XMPPResource>)resourceForJID:(XMPPJID *)jid
{
	...
	{
        //nehaa 26-04-14
        //change this for updating record of presence instead of creating new record
        jidStr = [[jidStr componentsSeparatedByString:@"/"] objectAtIndex:0];
        for (XMPPResourceCoreDataStorageObject *resource in [self resources])
        {
