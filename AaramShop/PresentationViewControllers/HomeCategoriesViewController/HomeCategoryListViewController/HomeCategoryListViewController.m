...
@interface HomeCategoryListViewController ()
{
	...
	NSUInteger makeFovouriteCounts,makeStoreFovouriteCount;
	NSString *homeStoreIdentifier,*recommenedStoreIdentifier;
	RecommendedStoreCell *recommendedCell;
	HomeCategoryListCell *cell;
...
{
	homeStoreIdentifier=@"HomeCategoryListCell";
	recommenedStoreIdentifier=@"RecommendedStoreCell";
	
	if (tableView == tblRecommendedStore)
	{
		
		//RecommendedStoreCell *recommendedCell = [tableView dequeueReusableCellWithIdentifier:recommenedStoreIdentifier];
		recommendedCell = [tableView dequeueReusableCellWithIdentifier:recommenedStoreIdentifier];
		
		if (!recommendedCell)
		{
			[tableView registerNib:[UINib nibWithNibName:recommenedStoreIdentifier bundle:nil] forCellReuseIdentifier:recommenedStoreIdentifier];
			
			recommendedCell = [tableView dequeueReusableCellWithIdentifier:recommenedStoreIdentifier];
		}
		//		int width = [UIScreen mainScreen].bounds.size.width - 95;
		[recommendedCell setRightUtilityButtons:[self leftButtons] WithButtonWidth:92];
		//        [cell setRightUtilityButtons:[self leftButtons] WithButtonWidth:225];
		
		recommendedCell.backgroundColor = [UIColor whiteColor];
		recommendedCell.isRecommendedStore = NO;
		
		StoreModel *objStoreModel = [arrRecommendedStores objectAtIndex:indexPath.row];
		//makeStoreFovouriteCount=indexPath.row;
		recommendedCell.indexPath=indexPath;
		recommendedCell.delegate=self;
		//cell.imgIsFavourite.tag=makeStoreFovouriteCount;
		recommendedCell.imgFavourite.tag=makeStoreFovouriteCount;
		NSLog(@" indexxxxxx number %lu",(unsigned long)makeFovouriteCounts);
		//cell.deletebtn.tag=deletedLocationListIndex;
		[recommendedCell updateCellWithData:objStoreModel];
//		NSMutableDictionary *dic;
//		objStoreModel = [arrRecommendedStores objectAtIndex:indexPath.row];
//		[dic setObject:objStoreModel.store_id forKey:kStore_id];
	...
	else
	{
		...
		//HomeCategoryListCell *cell = (HomeCategoryListCell *)[tableView dequeueReusableCellWithIdentifier:homeStoreIdentifier];
		cell = (HomeCategoryListCell *)[tableView dequeueReusableCellWithIdentifier:homeStoreIdentifier];
		
		if (cell == nil) {
			cell = [[HomeCategoryListCell alloc] initWithStyle:UITableViewCellStyleDefault reuseIdentifier:homeStoreIdentifier];
		}
		//		int width = [UIScreen mainScreen].bounds.size.width - 95;
		...
		cell.imgIsFavourite.tag=makeFovouriteCounts;
		NSLog(@" index number %lu",(unsigned long)makeFovouriteCounts);
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	//UITabBarController *tabBar = [appDeleg createTabBarRetailer];
     tabBar = [appDeleg createTabBarRetailer];
	
	tabBar.hidesBottomBarWhenPushed = YES;
	self.navigationController.navigationBarHidden = YES;
	NSLog(@"print store Id %@",appDeleg.objStoreModel.store_id);
...
- (void)swipeableTableViewCell:(SWTableViewCell *)cell didTriggerLeftUtilityButtonWithIndex:(NSInteger)index
{
	...
	BOOL isHomeCell = NO;
	if([recommendedCell isKindOfClass:[HomeCategoryListCell class]])
	{
		indexPath= [(UITableView *)tblStores indexPathForCell: recommendedCell];
		isHomeCell = YES;
	}
	else
	{
		indexPath= [(UITableView *)tblRecommendedStore indexPathForCell: recommendedCell];
		isHomeCell = NO;
	}
	StoreModel *objStoreModel;
...
- (void)responseReceived:(id)responseObject
{
	...
	{
		case TASK_TO_GET_STORES_FROM_CATEGORIES_ID:
		{
		...
		case TASK_TO_MAK_FAVORITE:
		{
			
			if ([[responseObject objectForKey:kstatus] intValue] == 1)
			{
				NSLog(@"Make Favourite Success");
			...
			break;
		case TASK_TO_MAK_STORE_FAVORITE:
		{
			
			if ([[responseObject objectForKey:kstatus] intValue] == 1)
			{
				NSLog(@"Make Favourite Recommended Store Success");
				appDeleg.objStoreModel.is_favorite = [NSString stringWithFormat:@"%d",[[responseObject objectForKey:@"isFavorite"] intValue]];
				[Utils showAlertView:kAlertTitle message:[responseObject objectForKey:kMessage] delegate:nil cancelButtonTitle:kAlertBtnOK otherButtonTitles:nil];
				//[self setUpNavigationBar];
			}
...
-(void)makeStoreFovourite:(NSInteger)index
{
	...
	if ([cell isKindOfClass:[HomeCategoryListCell class]]) {
		...
		makeFovouriteCounts = index;
		NSLog(@"Make Home Store Favourite");
		//AddressModel *addressModel = [datasource objectAtIndex:deletedLocationListIndex];
		NSLog(@"indexNumber %lu",(unsigned long)makeFovouriteCounts);
		dic = [Utils setPredefindValueForWebservice];
		//NSMutableDictionary *params=[[NSMutableDictionary alloc]init];
		// [params setObject:address_id  forKey:kUser_address_id];
		StoreModel *objStoreModel = [arrAllStores objectAtIndex:index];
		[dic setObject:objStoreModel.store_id forKey:kStore_id];
		...
		[self callWebServiceToMakeFavourite:dic];
		
	}
	
	
	else if ([recommendedCell isKindOfClass:[RecommendedStoreCell class]])
	{
		
		NSLog(@"Make Recommended Store Favourite");

		makeStoreFovouriteCount = index;
		//AddressModel *addressModel = [datasource objectAtIndex:deletedLocationListIndex];
		NSLog(@"indexNumberrrrrr %lu",(unsigned long)makeStoreFovouriteCount);
		NSMutableDictionary *dict = [Utils setPredefindValueForWebservice];
		//NSMutableDictionary *params=[[NSMutableDictionary alloc]init];
		 //[params setObject:address_id  forKey:kUser_address_id];
		StoreModel *objStoreModel = [arrRecommendedStores objectAtIndex:index];
		[dict setObject:objStoreModel.store_id forKey:kStore_id];
		//[dic setObject:appDeleg.objStoreModel.store_id forKey:kStore_id];
		//[params setObject:userId  forKey:kUserId];
		
		
		//[params setObject:addressModel.user_address_id forKey:@"user_address_id"];
		
		[self callWebServiceToMakeFavourite:dict];
	}

	else{
		NSLog(@"None of Cell Selected");
	}
	/*
	if ([cell is kindOfClass:[CustomCellClass1 class]] ) {
		[(CustomCellClass1 *)cell doWhatever];
...
-(void)callWebServiceToMakeFavourite:(NSMutableDictionary *)aDict
{
	[AppManager startStatusbarActivityIndicatorWithUserInterfaceInteractionEnabled:YES];
	if (![Utils isInternetAvailable])
	{
	...
	[aaramShop_ConnectionManager getDataForFunction:kURLMakeFavorite withInput:aDict withCurrentTask:TASK_TO_MAK_FAVORITE andDelegate:self];
}
