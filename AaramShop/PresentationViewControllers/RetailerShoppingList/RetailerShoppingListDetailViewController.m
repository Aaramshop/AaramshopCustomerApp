	...
	if ([arrProductList count]>0)
	{
		for (ProductsModel *productModel in arrProductList) {
			if([productModel.isAvailable intValue] == 1)
			{
				...
				[AppManager AddOrRemoveFromCart:[self getCartProductFromProduct:productModel] forStore:[NSDictionary dictionaryWithObjectsAndKeys:appDeleg.objStoreModel.store_id,kStore_id,appDeleg.objStoreModel.store_name,kStore_name,appDeleg.objStoreModel.store_image,kStore_image, nil] add:YES fromCart:NO fromReorder:NO];
			}
		}
		CartViewController *cartView = (CartViewController *)[[UIStoryboard storyboardWithName:@"Main" bundle:nil]instantiateViewControllerWithIdentifier:@"CartViewScene"];
		[self.navigationController pushViewController:cartView animated:YES];
